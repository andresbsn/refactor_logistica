...
export function StockInventoryDialog({ open, onOpenChange, materials }: StockInventoryDialogProps) {
  ...
  const getStockStatus = (material: Material) => {
    ...
    if (percentage <= 100) return { label: "Bajo", variant: "destructive" as const }
    if (percentage <= 150) return { label: "Medio", variant: "default" as const }
    return { label: "Óptimo", variant: "secondary" as const }
  }

  const getCategoryColor = (category: string) => {
    const colors: Record<string, string> = {
      Alumbrado: "bg-yellow-500/10 text-yellow-700 border-yellow-200",
      Calles: "bg-gray-500/10 text-gray-700 border-gray-200",
      Señalización: "bg-blue-500/10 text-blue-700 border-blue-200",
      Agua: "bg-cyan-500/10 text-cyan-700 border-cyan-200",
      Residuos: "bg-green-500/10 text-green-700 border-green-200",
    }
