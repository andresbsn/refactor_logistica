...
export function generateProximityRoutes(tickets, config = defaultConfig) {
  ...
  if (config.groupProximityByCategory) {
    ...
    // Para cada categoria, generar rutas por cercanía
    categoriesMap.forEach((categoryTickets, category) => {
      ...
      for (const ticket of categoryTickets) {
        ...
        // Buscar tickets cercanos de la misma categoría
    ...
    // Comportamiento original: solo cercanía sin importar categoría
    for (const ticket of sortedTickets) {
...
export function generateCategoryRoutes(tickets, config = defaultConfig) {
  const pendingTickets = tickets.filter((t) => t.status === "open")
  ...
  categories.forEach((categoryTickets, category) => {
    ...
    // Crear múltiples rutas si hay más tickets que el máximo
    ...
    while (remaining.length >= config.minTicketsForRoute) {
      ...
      routes.push({
        ...
        description: `${routeTickets.length} tickets de categoría ${category}`,
      })
      routeId++
      subRouteId++
    }
...
export function generateHybridRoutes(tickets, config = defaultConfig) {
  const pendingTickets = tickets.filter((t) => t.status === "open")
  if (pendingTickets.length === 0) return []
  ...
  for (const { ticket } of ticketsWithScore) {
    ...
    for (const { ticket: other } of ticketsWithScore) {
      if (used.has(other.id)) continue
...
export function generatePriorityRoutes(tickets, config = defaultConfig) {
  const pendingTickets = tickets.filter((t) => t.status === "open")
